...
public:
    ...
    [[nodiscard]] std::uint32_t select(std::span<const MtpAdaptiveSignal* const> signals,
        ...
        last_transition_from_ = selected_window_;
        last_transition_to_   = selected_window_;
        if (signals.empty() || signals.size() != available.size() ||
            signals.size() != room.size() || signals.size() != frontiers.size()) {
        ...
        if (maximum_window_ >= 6 && best_window < 6) {
            ...
            const std::uint32_t floor_window =
                widest_admissible_width(6, best_window, signals.size(), batch_frontier);
    ...
    [[nodiscard]] std::uint32_t transition_from() const noexcept { return last_transition_from_; }
...
private:
    ...
    startup_window(std::span<const MtpAdaptiveSignal* const> signals,
                   std::span<const std::uint32_t> available, std::span<const std::uint32_t> room,
