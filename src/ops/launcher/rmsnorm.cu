...
void launch_rmsnorm(const Tensor& x, const Tensor& weight, const Tensor* z, Tensor& out,
                    ...
                    hipStream_t stream) {
    const auto* x_bf16 = static_cast<const __hip_bfloat16*>(x.data);
    const auto* w_bf16 = static_cast<const __hip_bfloat16*>(weight.data);
    const auto* z_bf16 = z != nullptr ? static_cast<const __hip_bfloat16*>(z->data) : nullptr;
    auto* out_bf16     = static_cast<__hip_bfloat16*>(out.data);
    ...
    if (aligned2 && d == 128 && Epilogue == RmsEpilogue::Plain) {
        ...
        rmsnorm_d128_bf16x2_kernel<Epilogue, kBlock>
            <<<blocks, kBlock, 0, stream>>>(reinterpret_cast<const __hip_bfloat162*>(x_bf16),
                                            reinterpret_cast<const __hip_bfloat162*>(w_bf16),
                                            reinterpret_cast<const __hip_bfloat162*>(z_bf16),
                                            reinterpret_cast<__hip_bfloat162*>(out_bf16), rows, eps);
    } else if (aligned2 && d >= 64 && d <= 256 && d % 64 == 0) {
        constexpr int kBlock         = 512;
        constexpr int kWarpsPerBlock = kBlock / kWarpSize;
        const auto blocks = static_cast<unsigned int>((rows + kWarpsPerBlock - 1) / kWarpsPerBlock);
        rmsnorm_warp_bf16x2_kernel<Epilogue, kBlock><<<blocks, kBlock, 0, stream>>>(
            reinterpret_cast<const __hip_bfloat162*>(x_bf16),
            reinterpret_cast<const __hip_bfloat162*>(w_bf16),
            reinterpret_cast<const __hip_bfloat162*>(z_bf16),
            reinterpret_cast<__hip_bfloat162*>(out_bf16), d, rows, eps);
...
void rmsnorm_launch(const Tensor& x, const Tensor& weight, float eps, bool unit_offset,
    ...
    const bool aligned2 =
        ((x_addr | w_addr | z_addr | o_addr) & (alignof(__hip_bfloat162) - 1)) == 0;
