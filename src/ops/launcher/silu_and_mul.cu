...
bool can_use_dim0_split_fast_path(const Tensor& gate, const Tensor& up, const Tensor& out) {
    ...
    if (gate.nb[0] != static_cast<std::int64_t>(sizeof(__hip_bfloat16)) ||
        up.nb[0] != static_cast<std::int64_t>(sizeof(__hip_bfloat16))) {
        return false;
    }
    if ((gate.nb[1] % static_cast<std::int64_t>(sizeof(__hip_bfloat162))) != 0 ||
        (up.nb[1] % static_cast<std::int64_t>(sizeof(__hip_bfloat162))) != 0) {
        return false;
    }
    ...
    return ((gate_addr | up_addr | out_addr) & (alignof(__hip_bfloat162) - 1)) == 0;
}
...
void silu_and_mul_launch(const Tensor& gate, const Tensor& up, Tensor& out, hipStream_t stream) {
    ...
    if (!gate.is_contiguous() || !up.is_contiguous()) {
        if (can_use_dim0_split_fast_path(gate, up, out)) {
            ...
            for_each_token_slice(gate.ne[1], 1, [&](int token_offset, int token_count) {
                ...
                silu_and_mul_dim0_split_kernel<<<grid, kBlock, 0, stream>>>(
                    static_cast<const __hip_bfloat16*>(gate_slice.data),
                    static_cast<const __hip_bfloat16*>(up_slice.data),
                    static_cast<__hip_bfloat16*>(out_slice.data), gate.ne[0],
                    gate.nb[1] / static_cast<std::int64_t>(sizeof(__hip_bfloat16)),
                    up.nb[1] / static_cast<std::int64_t>(sizeof(__hip_bfloat16)));
        ...
        silu_and_mul_strided_input_kernel<<<scalar_grid, kBlock, 0, stream>>>(
            static_cast<const __hip_bfloat16*>(gate.data),
            static_cast<const __hip_bfloat16*>(up.data), static_cast<__hip_bfloat16*>(out.data), n,
    ...
    const auto up_addr   = reinterpret_cast<std::uintptr_t>(up.data);
    const auto out_addr  = reinterpret_cast<std::uintptr_t>(out.data);
    if (((gate_addr | up_addr | out_addr) & (alignof(__hip_bfloat162) - 1)) != 0) {
        const int scalar_grid = static_cast<int>(div_up(n, static_cast<std::int64_t>(kBlock)));
    ...
    silu_and_mul_kernel<<<grid, kBlock, 0, stream>>>(static_cast<const __hip_bfloat16*>(gate.data),
                                                     static_cast<const __hip_bfloat16*>(up.data),
                                                     static_cast<__hip_bfloat16*>(out.data), n);
    HIP_CHECK(hipGetLastError());
}
