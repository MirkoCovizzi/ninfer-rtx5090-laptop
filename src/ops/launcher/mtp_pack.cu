...
void mtp_pack_fc_input_launch(const Tensor& embedding_norm, const Tensor& hidden_norm, Tensor& out,
    ...
    for_each_token_slice(embedding_norm.ne[1], 1, [&](int token_offset, int token_count) {
        ...
        mtp_pack_fc_input_kernel<<<grid, kBlock, 0, stream>>>(
            static_cast<const __hip_bfloat16*>(embedding_slice.data),
            static_cast<const __hip_bfloat16*>(hidden_slice.data),
            static_cast<__hip_bfloat16*>(out_slice.data), embedding_norm.ne[0]);
...
void mtp_split_attn_in_launch(const Tensor& attn_in, Tensor& q, Tensor& k, Tensor& gate, Tensor& v,
    ...
    mtp_split_attn_in_kernel<<<grid, kBlock, 0, stream>>>(
        static_cast<const __hip_bfloat16*>(attn_in.data), static_cast<__hip_bfloat16*>(q.data),
        static_cast<__hip_bfloat16*>(k.data), static_cast<__hip_bfloat16*>(gate.data),
        static_cast<__hip_bfloat16*>(v.data), attn_in.ne[1]);
    HIP_CHECK(hipGetLastError());
}
