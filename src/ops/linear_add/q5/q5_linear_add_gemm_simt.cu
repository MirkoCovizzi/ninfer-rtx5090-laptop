...
void launch_split2(const Tensor& x, const Weight& w, Tensor& residual_out, hipStream_t stream) {
    ...
    q5_rowsplit_gemm_simt_split2_kernel<Q5RowSplitSimtSchedule, Cols, FullSlabs, Stride, false, 0,
        ...
        static_cast<__hip_bfloat16*>(residual_out.data), residual_out.ne[0], x.ne[0], x.ne[1],
        w.padded_shape[1], FullSlabs);
}

template <int Cols>
void dispatch_shape(const Tensor& x, const Weight& w, Tensor& residual_out, hipStream_t stream) {
    if (w.k == 6144) {
