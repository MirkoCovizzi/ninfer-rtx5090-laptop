...
void launch_tt(const Tensor& x, const Weight& w, Tensor& residual_out, hipStream_t stream) {
    ...
    const W8ContiguousOutput output{static_cast<__hip_bfloat16*>(residual_out.data), rows};
