...
void launch_variant(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    static_assert((kIntermediate % (Schedule::BM / 2)) == 0);
    const W8ContiguousOutput output{static_cast<__hip_bfloat16*>(out.data), kIntermediate};
