...
void launch_variant(const Tensor& x, const Weight& weight, Output output, hipStream_t stream) {
...
void launch_route(const Tensor& x, const Weight& weight, Output output, hipStream_t stream) {
    if ((x.ne[1] % kCols) == 0) {
