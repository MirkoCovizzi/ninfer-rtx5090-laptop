...
__device__ __forceinline__ std::uint32_t pack_bf16x2(float lo, float hi) {
    ...
    // HIP's __floats2bfloat162_rn packs (low, high) with round-to-nearest-even,
    // matching the CUDA cvt.rn.bf16x2.f32 semantics (hi -> upper half).
    const __hip_bfloat162 packed = __floats2bfloat162_rn(lo, hi);
    out                          = *reinterpret_cast<const std::uint32_t*>(&packed);
#else
...
__device__ __forceinline__ float2 bf16x2_to_float2(__hip_bfloat162 value) {
    return __bfloat1622float2(value);
}

__device__ __forceinline__ float2 bf16x2_bits_to_float2(std::uint32_t bits) {
    return bf16x2_to_float2(load_vec<__hip_bfloat162>(&bits));
}
