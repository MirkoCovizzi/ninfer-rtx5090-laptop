...
void launch_variant(const Tensor& x, const Weight& first_weight, const Weight& second_weight,
    ...
    auto* first_output        = static_cast<__hip_bfloat16*>(first_out.data);
    auto* second_output       = static_cast<__hip_bfloat16*>(second_out.data);
...
template <int TileCols, bool Full>
void launch_simt_single(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
    ...
    const W8ContiguousOutput output{static_cast<__hip_bfloat16*>(out.data), out.ne[0]};
    w8_rowsplit_gemm_simt_kernel<W8RowSplitSimtSchedule, TileCols, RowsPerCta, Stages, Full>
        <<<grid, RowsPerCta * 32, 0, stream>>>(static_cast<const __hip_bfloat16*>(x.data),
