...
void launch_q4_gemv(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
...
void launch_q4_simt(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
    const std::int32_t cols   = x.ne[1];
    const std::int32_t out_ld = static_cast<std::int32_t>(out.nb[1] / sizeof(__hip_bfloat16));
...
void launch_q4_simt_route(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
...
void launch_q4(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
    if (x.ne[1] == 1) {
...
void launch_q5_gemv(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
    ...
    q5_rowsplit_gemv_kernel<kValueZRows, kHidden, kRowsPerBlock, 2, true, false, true, kValueRows>
        <<<kValueZRows / kRowsPerBlock, kThreads, 0, stream>>>(
            ...
            static_cast<__hip_bfloat16*>(value.data), static_cast<__hip_bfloat16*>(z.data));
    HIP_CHECK(hipGetLastError());
}
...
void launch_q5_split4(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
    ...
    const std::int32_t out_ld = static_cast<std::int32_t>(value.nb[1] / sizeof(__hip_bfloat16));
    ...
    q5_rowsplit_gemm_simt_split4_kernel<Q5RowSplitSimtSchedule, Cols, 5, kHidden, true, kValueRows>
        <<<grid, kThreads, 0, stream>>>(static_cast<const __hip_bfloat16*>(x.data),
                                        static_cast<const std::uint8_t*>(weight.qdata),
                                        static_cast<const std::uint8_t*>(weight.qhigh),
                                        static_cast<const std::uint8_t*>(weight.scales),
                                        static_cast<__hip_bfloat16*>(value.data),
                                        static_cast<__hip_bfloat16*>(z.data), kValueZRows, out_ld,
...
void launch_q5_simt_r8_c8(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
    ...
    q5_rowsplit_gemm_simt_kernel<Q5RowSplitSimtSchedule, kColsPerTile, kRowsPerBlock, kStages, true,
        ...
        static_cast<const __hip_bfloat16*>(x.data), static_cast<const std::uint8_t*>(weight.qdata),
        static_cast<const std::uint8_t*>(weight.qhigh),
        static_cast<const std::uint8_t*>(weight.scales), static_cast<__hip_bfloat16*>(value.data),
        static_cast<__hip_bfloat16*>(z.data), kValueZRows, out_ld, kHidden, cols,
...
void launch_t4_pdl(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
                   Tensor& qk, Tensor& value, Tensor& z, hipStream_t stream) {
    ...
    const std::int32_t q4_out_ld = static_cast<std::int32_t>(qk.nb[1] / sizeof(__hip_bfloat16));
    const std::int32_t q5_out_ld = static_cast<std::int32_t>(value.nb[1] / sizeof(__hip_bfloat16));
    ...
    HIP_CHECK(pdl::launch_dependent(
        ...
        static_cast<const std::uint8_t*>(qk_weight.scales), static_cast<__hip_bfloat16*>(qk.data),
