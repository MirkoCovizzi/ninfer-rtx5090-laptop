...
template <int NStrip>
hipError_t launch_fixed(const state_passing_config& cfg, head_map qk_map, int NT) {
    using D = kernel::kernel_dims<NStrip>;
    constexpr int smem_bytes =
        kernel::smem_layout<NStrip>::SMEM_FLOATS * static_cast<int>(sizeof(float));

    hipError_t err = hipFuncSetAttribute(reinterpret_cast<const void*>(kernel::state_passing_kernel<NStrip>),
