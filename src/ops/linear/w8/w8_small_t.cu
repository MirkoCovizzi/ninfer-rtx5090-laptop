...
void launch_exact(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
    ...
    const W8ContiguousOutput output{static_cast<__hip_bfloat16*>(out.data), Geometry::kOutputRows};
...
void launch_w8_small_t(const Tensor& x, const Weight& weight, Tensor& out, hipStream_t stream) {
