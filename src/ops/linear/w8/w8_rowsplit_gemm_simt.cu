...
void launch_tt(const __hip_bfloat16* xp, const std::uint8_t* codes, const std::uint8_t* scales,
               __hip_bfloat16* outp, std::int32_t n, std::int32_t k, std::int32_t t,
...
void launch_slice(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    ...
    launch_tt<ColsPerTile, Full>(xp, static_cast<const std::uint8_t*>(w.qdata),
                                 ...
                                 static_cast<__hip_bfloat16*>(out.data), out.ne[0], x.ne[0], x.ne[1],
...
void launch_w8_simt_r8_c4(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    launch_route<4>(x, w, out, stream);
}

void launch_w8_simt_r8_c8(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
