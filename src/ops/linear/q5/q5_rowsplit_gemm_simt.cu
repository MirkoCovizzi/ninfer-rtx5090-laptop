...
void launch_simt(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    ...
    q5_rowsplit_gemm_simt_kernel<Q5RowSplitSimtSchedule, ColsPerTile, kRowsPerBlock, kStages>
        <<<grid, kThreads, 0, stream>>>(xp, static_cast<const std::uint8_t*>(w.qdata),
                                        ...
                                        static_cast<__hip_bfloat16*>(out.data), nullptr, rows,
                                        out_ld, k, cols, padded_k, full_slabs);
    HIP_CHECK(hipGetLastError());
}

template <int ColsPerTile>
void launch_simt_route(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
...
void launch_split2(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    ...
    q5_rowsplit_gemm_simt_split2_kernel<Q5RowSplitSimtSchedule, Cols, FullSlabs, Stride>
        <<<grid, kThreads, 0, stream>>>(
            ...
            static_cast<__hip_bfloat16*>(out.data), out.ne[0], x.ne[0], x.ne[1], w.padded_shape[1],
            FullSlabs);
}

template <int Cols>
void dispatch_split2_cols(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    if (w.k == 6144) {
...
template <int Cols>
void launch_split4(const Tensor& x, const Weight& w, Tensor& out, hipStream_t stream) {
    ...
    q5_rowsplit_gemm_simt_split4_kernel<Q5RowSplitSimtSchedule, Cols, 5, 5120>
        <<<grid, kThreads, 0, stream>>>(
            static_cast<const __hip_bfloat16*>(x.data), static_cast<const std::uint8_t*>(w.qdata),
            static_cast<const std::uint8_t*>(w.qhigh), static_cast<const std::uint8_t*>(w.scales),
            static_cast<__hip_bfloat16*>(out.data), nullptr, out.ne[0], out_ld, x.ne[0], x.ne[1],
            w.padded_shape[1], 5);
}
