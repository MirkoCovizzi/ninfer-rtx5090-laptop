...
__launch_bounds__(128, 2) __global__ void causal_attention_small_t_tc_partial_bf16_kernel(
    ...
    if constexpr (CacheInput::writes_cache) {
        input.k += static_cast<std::int64_t>(kCausalHeadDim) * Geometry::KVHeads * current_base;
        input.v += static_cast<std::int64_t>(kCausalHeadDim) * Geometry::KVHeads * current_base;
    }
...
#pragma unroll
        ...
        causal_small_t_tc_row_to_qt<Geometry>(row0, active_tokens, kv_head, q_head0, token0);
        causal_small_t_tc_row_to_qt<Geometry>(row1, active_tokens, kv_head, q_head1, token1);
