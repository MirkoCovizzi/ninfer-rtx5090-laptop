...
__device__ __forceinline__ __hip_bfloat162 residual_add_pair(__hip_bfloat162 y, __hip_bfloat162 x) {
    const float r0 = __low2float(x) + __low2float(y);
    const float r1 = __high2float(x) + __high2float(y);
    return __floats2bfloat162_rn(r0, r1);
}

__global__ void residual_add_scalar_kernel(const __hip_bfloat16* y, __hip_bfloat16* x,
                                           std::int64_t n) {
...
__launch_bounds__(256) __global__
    void residual_add_bf16x2_kernel(const __hip_bfloat16* y, __hip_bfloat16* x, std::int64_t n) {
