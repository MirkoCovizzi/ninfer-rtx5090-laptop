...
KvCacheStorage parse_kv_cache(std::string_view text) {
    ...
    if (text == "kvarn") { return KvCacheStorage::KvarnNvfp4V2Group64; }
    throw std::invalid_argument("--kv-dtype must be bf16, int8, or kvarn");
}
...
std::string usage_text(std::string_view program) {
    ...
    out << "Usage: " << program << " --weights <artifact.ninfer> [options]\n\n"
        ...
        << "  --kv-dtype <bf16|int8|kvarn> KV cache storage (default: bf16)\n"
